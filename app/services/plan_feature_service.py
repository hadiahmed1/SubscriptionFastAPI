...
async def add_plan_feature(planId,featureId, company:User=Depends(get_current_company))->PlanFeatures:
    ...
    if feature.company!=company.id or plan.companyId!=company.id:
        raise HTTPException(status_code=403, detail="Only Plan owner can add Feature to Plan")
    
    return await db.planfeatures.create(data={
        "featureId":featureId,
        "planId":planId
    })