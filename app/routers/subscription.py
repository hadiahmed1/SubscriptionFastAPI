...
async def post_subscription(plan_id, user: User = Depends(get_current_user))->Subscription:
    try:
...
async def get_subscriptions(user: User = Depends(get_current_user))->List[Subscription]:
